...
export default function AdminWorkoutsScreen({ route, navigation }) {
  ...
  const filtered = workouts.filter(w => {
    const matchSearch = w.title?.toLowerCase().includes(search.toLowerCase()) || w.user?.name?.toLowerCase().includes(search.toLowerCase());
    ...
    <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
      <View style={styles.cardTop}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.wMeta, { color: theme.textMuted }]}>{item.durationMinutes} min  •  {item.category}  •  {item.difficulty}</Text>
        </View>
        <View style={[styles.statusBadge, { backgroundColor: (STATUS_COLORS[item.status] || '#6366F1') + '22' }]}>
          <Text>{STATUS_ICONS[item.status] || '📅'}</Text>
          <Text style={[styles.statusText, { color: STATUS_TEXT[item.status] || '#818CF8' }]}>{item.status?.replace('_', ' ')}</Text>
        </View>
      </View>
      {item.exercises?.length > 0 && <Text style={[styles.exCount, { color: theme.accentText }]}>🏋️ {item.exercises.length} exercise{item.exercises.length !== 1 ? 's' : ''}</Text>}
      <View style={styles.cardActions}>
        <TouchableOpacity style={[styles.detailBtn, { backgroundColor: theme.blueLight, borderColor: theme.blue + '33' }]}
          onPress={() => navigation.navigate('AdminWorkoutDetail', { workout: item })}>
          <Text style={[styles.detailBtnText, { color: theme.blueText }]}>👁️  View Details</Text>
        </TouchableOpacity>
    ...
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <View style={[styles.searchWrap, { backgroundColor: theme.headerBg, borderBottomColor: theme.border }]}>
        <TextInput style={[styles.search, { backgroundColor: theme.inputBg, borderColor: theme.border, color: theme.textPrimary }]}
          placeholder="🔍  Search by title or user..." placeholderTextColor={theme.textMuted} value={search} onChangeText={setSearch} />
      </View>
      <View style={[styles.filterRow, { backgroundColor: theme.headerBg, borderBottomColor: theme.border }]}>
        {['all', 'planned', 'in_progress', 'completed', 'skipped'].map(s => (
          <TouchableOpacity key={s} onPress={() => setStatusFilter(s)}
            style={[styles.filterBtn, { backgroundColor: theme.card, borderColor: theme.border }, statusFilter === s && { backgroundColor: theme.accent, borderColor: theme.accent }]}>
            <Text style={[styles.filterText, { color: statusFilter === s ? '#fff' : theme.textMuted }]}>{s === 'all' ? 'All' : s.replace('_', ' ')}</Text>
          </TouchableOpacity>
        ))}
      </View>
      {loading ? <ActivityIndicator size="large" color={theme.accent} style={{ marginTop: 60 }} /> : (
        <FlatList data={filtered} keyExtractor={item => item._id} renderItem={renderWorkout}
          ...
          ListEmptyComponent={<View style={styles.empty}><Text style={styles.emptyIcon}>💪</Text><Text style={[styles.emptyText, { color: theme.textMuted }]}>No workouts found</Text></View>}
        />
      )}
    </View>
  );
